...
describe('legacy import', () => {
  it('dry-runs without writes, collapses duplicates, handles broken URLs, and reruns without overwriting edits', async () => {
    ...
    expect(await getSpec(imported.specId!, F.alice)).toMatchObject({ body: '# Imported schema', sourceType: 'git_import', specType: 'schema', status: 'in_review', links: [{ relationshipType: 'creates' }, { relationshipType: 'creates' }] })
    ...
    await updateSpec(imported.specId!, { specType: 'api', area: 'quotas', status: 'active' }, F.alice)
    expect(await getSpec(imported.specId!, F.alice)).toMatchObject({ specType: 'api', area: 'quotas', status: 'active' })
