...
describe('decisions pinned to a version', () => {
  ...
  it('marks an approval stale when content changes, but not for a status-only change', async () => {
    ...
    // Activating keeps v1: the approval and the version shown agree.
    expect(s).toMatchObject({ currentVersion: 1, lastApprovedVersion: 1, approvedNow: true })
    expect(s.history[0].state).toBe('approved')
    await updateSpec(spec.id, { specType: 'schema' }, F.alice)
    expect(await getReviewSummary('spec', spec.id, F.alice)).toMatchObject({ currentVersion: 1, approvedNow: true })
    await updateSpec(spec.id, { body: 'Tokens last 2h.' }, F.alice)
    s = await getReviewSummary('spec', spec.id, F.alice)
    expect(s).toMatchObject({ currentVersion: 2, lastApprovedVersion: 1, approvedNow: false })
