...
- **Spec status:** add `in_review` to the spec status union (`draft → in_review → active`). Requesting a review moves `draft` to `in_review`. Approval moves it to `active` if the product's workflow settings say so (see 3c); otherwise the author activates it from the spec page's action bar. Status changes never create a version: only a title or body change does, so an approval of v3 still reads as v3 once the spec is active.
